...
export default class CardComponent extends Component {
	render() {
		const { data } = this.props; // Data in Feed
		const { image } = JSON.parse( data.json_metadata ); // Parsing to image url in json_metadata
			...
			<Card transparent>
				...
				<CardItem style={ { height:45 } }>
					<Left>
						...
						<Button transparent>
							<Icon name='ios-send' style={ { color:'black', marginRight: 5 } }/>
						</Button>
						<Button transparent>
							<Icon name='gift' style={ { color:'black' } }/>
						</Button>
					</Left>
					<Right>
